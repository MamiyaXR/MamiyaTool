...
namespace MamiyaTool {
    public sealed partial class DebuggerComponent : MonoBehaviour {
        private abstract class ScrollableDebuggerWindowBase : IDebuggerWindow {
            ...
            protected static string GetRegularPath(string path) {
                return path?.Replace('\\', '/');
            }
            protected static float GetInchesFromPixels(float pixels) {
                return pixels / Screen.dpi;
            }
            protected static float GetCentimetersFromPixels(float pixels) {
