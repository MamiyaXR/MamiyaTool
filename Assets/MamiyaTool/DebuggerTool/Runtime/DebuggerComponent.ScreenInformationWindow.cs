        ...
        private sealed class ScreenInformationWindow : ScrollableDebuggerWindowBase {
            protected override void OnDrawScrollableWindow() {
                ...
                GUILayout.BeginVertical("box");
                {
                    ...
                    DrawItem("Screen Width", string.Format("{0} px / {1} in / {2} cm", Screen.width.ToString(), GetInchesFromPixels(Screen.width).ToString("F2"), GetCentimetersFromPixels(Screen.width).ToString("F2")));
                    DrawItem("Screen Height", string.Format("{0} px / {1} in / {2} cm", Screen.height.ToString(), GetInchesFromPixels(Screen.height).ToString("F2"), GetCentimetersFromPixels(Screen.height).ToString("F2")));
